...
namespace Cactus.Chat.Signalr
{
    public class ClientEndpoint<T> : IChatClientEndpoint where T : Hub
    {
        ...
        public ClientEndpoint(IHubContext<T> hubContext, string connectionId, ILogger log)
        {
            _hub = hubContext;
        ...
        protected Task SendAsync(string method, object model)
        {
            ...
            {
                _log.LogError(
                    "Something goes wrong while sending {method} to {connection_id}, the exception is {exception}",
                    method, _connectionId, ex);
                throw;
            }
        }
        ...
        public Task ParticipantAdded<T1>(string chatId, string userId, string participantId, T1 profile)
            where T1 : IChatProfile
        {
